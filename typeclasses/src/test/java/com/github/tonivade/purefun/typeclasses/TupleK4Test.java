...
public class TupleK4Test {
  ...
  final Functor<Option_> functor = Instance.functor(Option_.class);
  ...
  public void tuple() {
    ...
    assertAll(() -> assertEquals(TupleK.of(some("value"), some(10), some(LocalDate.of(2018, 11, 5)), none()), tuple),
              () -> assertEquals(TupleK.of(some("VALUE"), some(10), some(LocalDate.of(2018, 11, 5)), none()), tuple.map1(functor, String::toUpperCase)),
              () -> assertEquals(TupleK.of(some("value"), some(100), some(LocalDate.of(2018, 11, 5)), none()), tuple.map2(functor, i -> i * i)),
              () -> assertEquals(TupleK.of(some("value"), some(10), some(LocalDate.of(2018, 11, 6)), none()), tuple.map3(functor, date -> date.plusDays(1))),
              () -> assertEquals(TupleK.of(some("value"), some(10), some(LocalDate.of(2018, 11, 5)), none()), tuple.map4(functor, Object::toString)),
