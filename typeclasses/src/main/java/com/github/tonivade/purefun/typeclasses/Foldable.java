...
public interface Foldable<F extends Witness> {
  ...
  default <A> A fold(Monoid<A> monoid, Kind<F, A> value) {
    return foldMap(monoid, value, identity());
  }

  default <A, B> B foldMap(Monoid<B> monoid, Kind<F, A> value, Function1<A, B> mapper) {
    return foldLeft(value, monoid.zero(), (acc, a) -> monoid.combine(acc, mapper.apply(a)));
  }

  default <A> Option<A> reduce(Kind<F, A> value, Operator2<A> combinator) {
  ...
  default <G extends Witness, A, B> Kind<G, B> foldM(
      Monad<G> monad, Kind<F, A> value, B initial, Function2<B, A, Kind<G, B>> mapper) {
