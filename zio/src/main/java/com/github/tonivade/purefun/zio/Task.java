...
public final class Task<T> {
  ...
  public Future<Either<Throwable, T>> toFuture() {
    return value.toFuture(nothing());
  }

  public void async(Executor executor, Consumer1<Try<Either<Throwable, T>>> callback) {
    value.provideAsync(executor, nothing(), callback);
  }

  public void async(Consumer1<Try<Either<Throwable, T>>> callback) {
    value.provideAsync(nothing(), callback);
  }

  public <F extends Kind> Higher1<F, Either<Throwable, T>> foldMap(MonadDefer<F> monad) {
