...
public class FunctorLaws {
  ...
  public static <W extends Kind, F extends Mappeable<W, String>> void verifyLaws(F functor) {
    assertAll(() -> identity(functor),
              () -> composition(functor),
              () -> associativity(functor));
  }

  private static <W extends Kind, F extends Mappeable<W, String>> void identity(F functor) {
    assertEquals(functor, functor.map(Function1.identity()), "identity law");
  }

  private static <W extends Kind, F extends Mappeable<W, String>> void composition(F functor) {
    assertEquals(functor.map(toUpperCase).map(toLowerCase),
                 functor.map(toUpperCase.andThen(toLowerCase)),
                 "composition law");
  }

  private static <W extends Kind, F extends Mappeable<W, String>> void associativity(F functor) {
