...
public class MonadLaws {
  ...
  public static <W extends Witness, M extends Monad<W, String>> void verifyLaws(M monad, Function1<String, M> unit) {
    assertAll(() -> leftIdentity(monad, unit),
              () -> rightIdentity(monad, unit),
              () -> associativity(monad, unit));
  }

  private static <W extends Witness, M extends Monad<W, String>> void leftIdentity(M monad, Function1<String, M> unit) {
    assertEquals(monad.map(toUpperCase),
                 monad.flatMap(value -> unit.apply(toUpperCase.apply(value))),
                 "left identity law");
  }

  private static <W extends Witness, M extends Monad<W, String>> void rightIdentity(M monad, Function1<String, M> unit) {
    assertEquals(monad,
                 monad.flatMap(value -> unit.apply(value)),
                 "right identity law");
  }

  private static <W extends Witness, M extends Monad<W, String>> void associativity(M monad, Function1<String, M> unit) {
