...
public class TaglessTest {
  ...
  public void stateInterpreter() {
    ...
    assertEquals(ImmutableList.of("what's your name?", "Hello Toni"), run.get1());
  }
...
class Program<F extends Kind, P extends IOProgramT<F> & Monad<F>> {
  ...
  Higher1<F, Nothing> echo() {
    return program.flatMap(readName(), name -> program.write("Hello " + name));
  }

  private Higher1<F, String> readName() {
    return program.flatMap(program.write("what's your name?"), i -> program.read());
  }
}
