...
public class TryHandlerTest {
  ...
  public void mapEmptyTest() {
    ...
    assertEquals(Try.failure("error").isFailure(), str2int.map(a -> a * 2).handle("asdfg").isFailure());
  }
  ...
  public void filterEmptyTest() {
    ...
    assertEquals(Try.failure("error").isFailure(), str2int.filter(x -> x > 10).handle("asdfg").isFailure());
  }
  ...
  public void flatMapEmptyTest() {
    ...
    assertEquals(Try.failure("error").isFailure(), str2int.flatMap(a -> Try.failure("error")).handle("asdfg").isFailure());
  }
}
