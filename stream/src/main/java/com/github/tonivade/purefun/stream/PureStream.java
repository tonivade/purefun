...
@HigherKind
public sealed interface PureStream<F extends Witness, T> extends PureStreamOf<F, T>, Bindable<Kind<PureStream_, F>, T> permits Cons, Suspend, Nil {
  ...
  @Override
  <R> PureStream<F, R> flatMap(Function1<? super T, ? extends Kind<Kind<PureStream_, F>, ? extends R>> map);
  @Override
  default <R> PureStream<F, R> andThen(Kind<Kind<PureStream_, F>, ? extends R> next) {
  ...
  default <G extends Witness, R> PureStream<G, R> through(Function1<PureStream<F, T>, PureStream<G, R>> function) {
