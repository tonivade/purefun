...
public class ApplicativeLaws {
  ...
  public static <F> void verifyLaws(Applicative<F> applicative) {
  ...
  private static <F, A> void identityLaw(Applicative<F> applicative, Kind<F, A> value) {
    assertEquals(value, applicative.ap(value, applicative.pure(identity())), "identity law");
  }

  private static <F, A, B> void homomorphismLaw(Applicative<F> applicative, A value, Function1<A, B> f) {
    assertEquals(
  ...
  private static <F, A, B> void interchangeLaw(Applicative<F> applicative, A value, Function1<A, B> f) {
    assertEquals(
        applicative.ap(applicative.pure(value), applicative.pure(f)),
  ...
  private static <F, A, B> void mapLaw(Applicative<F> applicative, Kind<F, A> value, Function1<A, B> f) {
    assertEquals(
