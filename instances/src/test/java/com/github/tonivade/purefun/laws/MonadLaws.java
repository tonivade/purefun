...
public class MonadLaws {
  ...
  public static <F> void verifyLaws(Monad<F> monad) {
  ...
  private static <F, A, B> void leftIdentity(Monad<F> monad, Kind<F, A> value, Function1<A, B> f) {
    assertEquals(monad.map(value, f),
                 monad.flatMap(value, a -> monad.pure(f.apply(a))),
                 "left identity law");
  }

  private static <F, A> void rightIdentity(Monad<F> monad, Kind<F, A> value) {
    assertEquals(value,
                 monad.flatMap(value, monad::<A>pure),
                 "right identity law");
  }

  private static <F, A, B, C> void associativity(Monad<F> monad,
                                                              Kind<F, A> value,
                                                              Function1<A, B> f,
                                                              Function1<B, C> g) {
